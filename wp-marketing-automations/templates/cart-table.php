    ...
    <table cellspacing="0" cellpadding="6" border="1" width="100%">
        ...
        <?php if ( wc_tax_enabled() && $tax_display === 'excl' && $subtotal_tax ) : ?>
            <tr>
                ...
                <td><?php echo wp_kses_post( BWFAN_Common::price( $subtotal_tax + ( $shipping_tax ?? 0 ), $currency ) ); ?></td>
            </tr>
        <?php endif; ?>
        ...
        <tr>
            <th scope="row" <?php echo $colspan_foot; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> style="<?php echo esc_html( $text_align ); ?>">
		        ...
		        <?php if ( false === BWFAN_Merge_Tag_Loader::get_data( 'is_preview' ) && wc_tax_enabled() && $tax_display !== 'excl' ) : ?>
                    <small><?php printf( __( '(includes %s Tax)', 'woocommerce' ), BWFAN_Common::price( $subtotal_tax + ( $shipping_tax ?? 0 ), $currency ) );// phpcs:ignore WordPress.WP.I18n.TextDomainMismatch, WordPress.WP.I18n.MissingTranslatorsComment, WordPress.Security.EscapeOutput.OutputNotEscaped ?></small>
				<?php endif; ?>
            </th>
            <td>
		        <?php
